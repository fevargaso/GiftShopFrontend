...
export class AppComponent {
  ...
  protected readonly $cartItems = toSignal(this.cartService.cart$, { initialValue: [] as CartItem[] });

  protected readonly $carrCount = computed(() => this.$cartItems().reduce((total, item) => total + item.quantity, 0));

  protected readonly isLoginPage = computed(() => this.currentUrl()?.includes('/login'));

  protected readonly showSplash = computed(
    () => this.$app()!.initializing || !this.$app()!.operational || this.$app()!.checking,
  );

  protected readonly noOperational = computed(() => !this.$app()!.initializing && !this.$app()!.operational);
  ...
  protected readonly $cartCount = computed(() => this.$cartItems().reduce((total, item) => total + item.quantity, 0));
